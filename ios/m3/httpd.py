...
class HTTPServer:
    ...
    def get_data(self, uri):
        ...
        '''
        if len(uri) <= 1:
            first_page = """<button onclick=location.replace("/www?val=true") value = "True">Directory Mode</button>"""
            self.mode = True
            return (200, "text/html", len(first_page), first_page.encode("utf-8"))
        else:
            print(uri)
            if "=" in uri:
                dir_mode = uri.split("?")[1].split("=")[1]
                pos = uri.index("?")
                uri = uri[0:pos]
                print(uri)
                if dir_mode == "true":
                    if os.path.exists(os.getcwd()+uri):
                        ...
                        if isdir:
                            ...
                            filepaths = ""
                            for file in files:
                                filepaths += "<a href ="+uri+"/"+file+"?val=true"+">"+file+"</a>"+"</br>"
                            return (200, "text/html", len(filepaths),filepaths.encode("utf-8"))
                        else:
