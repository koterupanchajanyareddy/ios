...
class HTTPServer:
    ...
    def user(self, conn, addr):
        with conn:
            print('Connected by', addr)
            print("in the user mentod...so threading works")
            ...
            # print(code, c_type, c_length, data)
            response = self.response_headers(
                code, c_type, c_length) + data
    ...
    def __init__(self, IP, port):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM, socket.IPPROTO_TCP) as self.s:
            ...
            while True:
                ...
                t1 = threading.Thread(target=self.user,args=(conn, addr))
                t1.start()
                threads = threading.current_thread()
                print("shows same mainthreads all the time because of change of ip address due to reconnection of browser ever reload")
                print(threads)
    ...
    def get_data(self, uri):
        ...
        if uri == "/":
            data = ""
            files = os.listdir(os.getcwd()+uri)
            for file in files:
                data += "<a href="+uri+file+">"+file+"</a><br/>"
            return (200, "text/html", len(data), data.encode("utf-8"))
        if "bin" in uri:
            i = uri.find("/bin/")
            suri = uri[i +5:]
            print(suri)
            r,w = os.pipe()
            pid = os.fork()
            fileType = mimetypes.MimeTypes().guess_type(os.getcwd()+"/"+"op.txt")[0]
            if pid == 0:
                sys.stdout = open('op.txt','w')
                syscall = os.system(suri+"> op.txt")
                if not syscall:
                    pass
                else:
                    if not os.path.exists(os.getcwd()+"/"+suri):
                        data = "File Not Found"
                        os.write(w, data.encode("utf-8"))
                        sys.exit(0)
                    else:
                        exec(open(os.getcwd() + "/"+suri).read())
                sys.stdout = sys.__stdout__
                data = open(os.getcwd()+"/"+"op.txt", 'rb').read()
                os.write(w, data)
                sys.exit(0)
            else:
                # print("parent")
        ...
        if os.path.exists(os.getcwd()+uri):
            isdir = os.path.isdir(os.getcwd()+uri)
            print(" isdir = ", isdir)
            if isdir:
                files = os.listdir(os.getcwd()+uri)
                fileLinks = ""
                for file in files:
                    fileLinks += "<a href = "+uri+"/"+file+">"+file+"</a>"+" <br/>"
                return (200, "text/html", len(fileLinks), fileLinks.encode("utf-8"))
            else:
                print(os.getcwd()+uri, " file ")
                fileType = mimetypes.MimeTypes().guess_type(uri)[0]
                if fileType == None:
                    data = "<h1>None Type File</h1>"
                    return (200, "text/html", len(data), data.encode("utf-8"))
                data = open(os.getcwd()+uri, 'rb').read()
